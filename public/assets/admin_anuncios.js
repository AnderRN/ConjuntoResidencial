document.addEventListener('DOMContentLoaded', ()=>{
  ...
  async function load(){
    ...
    const res = await fetch('/api/admin/anuncios', { headers: { Authorization: 'Bearer '+token }});
    ...
    cardsAdmin.querySelectorAll('.edit').forEach(b=> b.addEventListener('click', async (e)=>{
      ...
      const r = await fetch('/api/admin/anuncios/'+id, { method:'PATCH', headers: { 'Content-Type':'application/json', Authorization: 'Bearer '+token }, body: JSON.stringify({ titulo: newTitle, descripcion: newDesc })});
      const jj = await r.json();
      if(jj.ok) load();
      else alert(jj.msg||'Error');
    }));
  ...
  btn.addEventListener('click', async ()=>{
    ...
    const r = await fetch('/api/admin/anuncios', { method:'POST', headers: { 'Content-Type':'application/json', Authorization: 'Bearer '+token }, body: JSON.stringify({ titulo, descripcion }) });
